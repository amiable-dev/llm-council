...
| `LLM_COUNCIL_FILE_SELECTION` | ADR-053 Q1 verify file classification: `content` (**default since the #557 flip** — git's NUL heuristic + snapshot `.gitattributes`, reviews any text file incl. unlisted languages), `allowlist` (the pre-flip extension list — the one-var opt-out restoring old behavior exactly), or `shadow` (acts on allowlist, records what content would change to `.council/selection/decisions.jsonl`). Invalid ⇒ content. The secret denylist (#548) and garbage filter always run first regardless | content |
...
| `LLM_COUNCIL_COVERAGE_POLICY` | ADR-053 #556 coverage clamp: **`clamp` is the default since the #557 flip** — a `pass` over an unreviewed changed/explicit file becomes `unclear(incomplete_coverage)`. `fail` raises a hard 422 instead; `warn` is the opt-out (receipt only, no verdict effect) and is **refused by `gate`** post-flip. Invalid ⇒ `clamp` | clamp |
