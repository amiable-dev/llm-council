...
Council core
- `council.py` — the two orchestrators (`run_council_with_fallback`, `run_full_council`), config helpers with patched-attr test semantics, deprecated-constant `__getattr__`. Split below the review cap (ADR-046 P0, #408): stage functions live in `council_stages.py` (`stage1_collect_responses[_with_status]`, `stage1_5_normalize_styles`, `stage2_collect_rankings`, `stage3_synthesize_final`, `quick_synthesis`), ranking/Borda/shadow-votes in `council_rankings.py` (`parse_ranking_from_text`, `calculate_aggregate_rankings`), shared constants + ADR-011 usage accounting in `council_usage.py`. **council.py re-exports every moved name**, so `llm_council.council.X` imports and patches keep working; patches on names CONSUMED inside stage functions (e.g. `query_models_parallel`) must target `llm_council.council_stages.*`. Moved code reads config helpers through the council module at call time, preserving `patch("llm_council.council.CHAIRMAN_MODEL", …)`. **Stage-timeout GOTCHA (#648):** `stage2_collect_rankings` and `stage3_synthesize_final` each default `timeout=120.0`, and for a long time BOTH consult orchestrators simply omitted the argument — so peer review and chairman synthesis ran at 120s regardless of tier, and `LLM_COUNCIL_TIMEOUT_MULTIPLIER` (which correctly reaches `TierContract.per_model_timeout_ms`) never reached either stage. That was invisible until the chairman became `claude-opus-5` (#635) and every reasoning-tier consult lost its synthesis. Both orchestrators now pass `resolve_stage_timeout(per_model_timeout)` (`council_usage.py`) = `max(per_model, TIMEOUT_STAGE_FLOOR=120)`; the floor exists because high tier's per-model budget is 90s and balanced's 45s, so the literal ADR-040 waterfall cap would have SHRUNK those stages. `run_full_council` is tier-agnostic and takes an optional `per_model_timeout` (None ⇒ the floor); the HTTP `/v1/council/run` endpoint passes the high-tier contract's value, the tier it already assumes for its evidence budget. **A new call to either stage function must pass `timeout` explicitly** — `tests/test_issue648_consult_stage_timeouts.py` AST-pins every call site in `council.py`, because the original defect was an omitted kwarg that reads as correct code.
- `consult_render.py` (#660) — pure presentation helpers for the consult surface: `synthesis_heading` / `degradation_notice` / `status_block` / `render_consult_body`. Exists because `consult_council` hardcoded `### Chairman's Synthesis` for EVERY outcome with the "N of M models" disclosure appended *below* the content, so a chairman-failed fallback (one member's raw text) was rendered with the authority of a completed council. Invariants: the heading is a function of `metadata["synthesis_type"]`/`status` and never defaults to the success heading on unknown input; the notice precedes the body; the JSON status block is emitted **unconditionally** (a block that appears only on degradation is itself something a caller must detect). GOTCHA: `synthesis_type` gained `single_model_raw` — set in `run_council_with_fallback` only when `quick_synthesis` reports `outcome["chairman"] == "failed"`; the old code labelled that case `partial`, which is why it read as deliberation. `quick_synthesis` cannot label its own output, so it reports facts back through the caller-supplied `outcome` dict (same pattern as `shared_raw_responses`). Its chairman call was a hardcoded `timeout=15.0` — the ONLY synthesis attempt on the global-timeout path, so a heavyweight chairman that had just blown a 90s stage-1 budget was re-asked with 15s; call sites now pass the tier budget and are AST-pinned in `tests/test_issue660_honest_degradation.py` (#648's defect class). Dissent for `verdict_type="synthesis"` lives in `metadata["dissent"]` and was never read by the MCP surface (rendered only inside the ADR-025b verdict block) — `include_dissent=true` was a silent no-op in the DEFAULT verdict type.
- `openrouter.py` — `query_model` / `query_models_parallel`; returns `{content, reasoning_details?}`; returns `None` on failure (graceful degradation). Per-model timeouts ARE independent (`asyncio.wait_for` per task, then gathered) — a recurring misdiagnosis; the tier total is enforced separately as `run_council_with_fallback`'s `synthesis_deadline`.
...
- `graduated_depth.py` (P3) — decision engine + opt-in hook (`plan_escalation`) for the depth ladder single→mini(3)→full: prefix-superset model sets (shallow responses always reusable, escalation only ADDS models), escalation gated on known-low CSS/confidence (unknown never escalates), ADR-011 estimator prices the added rung, opt-in BudgetEnforcer can veto (auditable, never a silent downgrade), approved escalations emit `L2_DELIBERATION_ESCALATION`. `merge_usage_summaries` sums usage across rungs (cost_known ORs). Active ladder still unwired (slice 2, gated on shadow data). **#618 shadow telemetry IS wired** into both orchestrators post-rankings: `evaluate_and_log_shadow_depth` classifies every full-depth run ("was full depth necessary?" — `mini_would_suffice`/`premature_halt_risk`/`would_escalate`/…) using internally-computed CSS (deliberately independent of `LLM_COUNCIL_QUALITY_METRICS` — that's an output-annotation flag) plus a counterfactual mini-rung CSS (subset re-rank of the mini prefix's reviews; approximation, both error directions observable), and appends JSONL to `.council/depth/decisions.jsonl` (numeric signals only, soft-fail, response byte-identical). Enablement criterion for slice 2: `premature_halt_risk` rate. GOTCHA (metric, ADR-036): unanimous clean rankings score CSS exactly 0.7 (the threshold boundary — winner margin caps at 0.25 under uniform spacing) and an all-tied aggregate returns 1.0 ("consensus on equality"), so rotational disagreement reads as perfect consensus.
...
- `observability/posthog_emitter.py` + `ai_generation.py` + `adjudication.py` (ADR-050, epic #472) — **opt-in, off-by-default** PostHog LLM-analytics emission. `posthog_emitter` (D1): `POSTHOG_API_KEY`/`POSTHOG_HOST` (EU ingestion `eu.i.posthog.com`), import-guarded lazy client (the `[posthog]` extra is never imported when unconfigured ⇒ byte-identical), non-blocking soft-fail `emit()`, bounded idempotent `shutdown()`, `scrub_exception` (D3, logs type only). `ai_generation` (D2): maps `usage.by_model` → one `$ai_generation` per member, `$ai_trace_id=verification_id`, **cache-subtraction clamp** `$ai_input_tokens=max(0,prompt−cache_read)` (PostHog hit-rate tile assumes exclusive counting), `$ai_total_cost_usd`=ADR-011 ground truth when `cost_known`; allowlist mapper (D3, never emits `$ai_input`/`$ai_output_choices`); wired soft-fail into `verification/api.py` (`_emit_posthog_generations`, gated before import). `adjudication` (D4): `emit_adjudication` posts a follow-up `$ai_metric` keyed to `$ai_trace_id` (numeric value + `adjudication_label`) — the epic-loop retro hook, NOT PostHog's automated Evaluations. GOTCHA: `$ai_metric` value-type/scope is verify-before-impl (public docs don't pin it). Display side (project `amiable-llm-ops`/212129, dashboard 793592) pre-exists. Part 4 (feature-flag tier routing) deferred to a future ADR.
...
- **Findings channel / mechanical gate (ADR-051, epic #484, C1–C3 in v0.36.0):** `verification/findings.py` — opt-in `LLM_COUNCIL_STRUCTURED_FINDINGS` (default OFF, explicit true-set; flag-off byte-identical). The chairman's BINARY-verdict JSON gains a findings-first `findings[]`; `parse_findings` extracts it via an LLM-resilient string-aware balanced-brace scanner (`_extract_json_object`, `preferred_key="findings"`) and soft-fails to the legacy prose path. `verdict_policy(findings)` is the **mechanical gate**: verdict = pure function of findings (any `critical` ⇒ `fail`), computed in `build_verification_result` when `findings_source == "structured"` (else legacy); `blocking_issues` = critical subset (`List[BlockingIssueResponse]`, no type break). **Severity is normalized fail-safe** (`_normalize_severity`: missing/unrecognized/typo/blocker-ish ⇒ `critical`; only explicit non-critical synonyms downgrade). Models in `schemas.py` (`Finding`, `VerifyDiagnostics`); threaded into the result via `api.py`. C4 (#488) adds `diagnostics.findings_by_severity` + a defensive `verdict_evidence_mismatch` invariant marker; C5 (#489) adds `diagnostics.inner_verdict`/`inner_confidence`/`inner_confidence_calibrated` (structured verdict BEFORE the low-confidence UNCLEAR softening) and makes `build_verification_result`'s mechanical block mutate `result` atomically (calibrate once, throw-free apply — a mid-block error leaves the legacy result intact). C6 (#490) documents the full response contract in `docs/guides/verify.md` and adds `TestVerifyResponseFieldDrift` (`tests/test_docs_drift.py`): every `VerifyResponse`/`Finding`/`VerifyDiagnostics` field must appear by name in `verify.md` or `api.md` or CI reds. Default-ON flip is a later breaking release. Spec: `docs/adr/ADR-051-implementation-spec.md`.
...
| `LLM_COUNCIL_TIMEOUT_MULTIPLIER` | Scales every tier timeout (`timeouts.multiplier`, 0.1–10.0, default 1.0). Since #648 it also reaches the consult path's stage 2/3 budgets. NOT the verify path's `VERIFICATION_TIMEOUT_MULTIPLIER` (a fixed 2.0 code constant) |
...
| `LLM_COUNCIL_GRADUATED_DEPTH` | ADR-044 P3: graduated deliberation depth ladder single→mini→full with consensus-gated escalation + budget veto (default off). #618 shadow telemetry (`.council/depth/decisions.jsonl`) records regardless of the flag |
...
| `LLM_COUNCIL_FILE_SELECTION` (`content`\|`allowlist`\|`shadow`) | ADR-053 Q1 verify file classification; **default `content` since the #557 flip** (NUL heuristic + snapshot `.gitattributes` + linguist + `.llmignore`). `allowlist` = pre-flip opt-out |
