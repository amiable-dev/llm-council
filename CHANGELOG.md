...
- **`consult_council(on_partial=…)` lets a caller decide what an incomplete council does ([#660](https://github.com/amiable-dev/llm-council/issues/660))** — `"synthesise"` (default, today's behaviour), `"error"` (return a structured `council_incomplete` blob **instead of** an answer, with no synthesis text to accidentally act on), or `"return_raw"` (the surviving members' responses attributed individually, no chairman synthesis over the top). Honest labelling helps a human reading the output; it does nothing for an automated caller that will act on the text either way, and the models that drop out are the slowest — hence generally the strongest — so a short council is skewed rather than merely smaller. An unrecognised value is **rejected** with `invalid_on_partial` rather than silently defaulting: `confidence` does silently fall back to `high`, but doing that here would turn a typo'd strictness request into permissiveness.

### Changed

- **`council_health_check` now probes the chairman by default ([#660](https://github.com/amiable-dev/llm-council/issues/660))** — **`deep` defaults to `true`.** [#596](https://github.com/amiable-dev/llm-council/issues/596) added the probe but left it off, so the default check still did not exercise the component its own docstring calls "the single point of failure that turns an otherwise-successful run into a verdict-less one", and `ready: true` went on being a green light that did not predict the outcome. There is no way to make `ready` chairman-conditional without a chairman call, so the honest default is to make it. A new top-level `ready_scope` (`connectivity_only` | `chairman_probed`) sits beside `ready` — widened only once a probe has actually run, never derived from the `deep` argument, so a skipped probe cannot claim coverage it doesn't have. The chairman probe is skipped when general connectivity has already failed: it adds nothing then, and billing for a second failing call during an outage is the wrong move. **Migration:** the default health check now costs one extra small model call (~2-3s); pass `deep=false` for the previous behaviour.
...
- **Type debt: 92 → 71 mypy errors, with a ratchet so it cannot climb back ([#642](https://github.com/amiable-dev/llm-council/issues/642))** — the mechanical tier of that ticket's plan. `no_implicit_optional` codemod (PEP 484 implicit-Optional, 6 files), `types-PyYAML` added to dev deps (3 missing-stub errors), and six missing container annotations. Two of the fixed errors were real if small defects rather than annotation noise: `metadata/selection.py` annotated `_create_synthetic_model_info(...) -> "ModelInfo"` with the name never in scope, so the string annotation was silently unchecked rather than enforced; and `gateway/cost_resolver.py` guarded `self._pricing_lookup(...)` with `if self._pricing_lookup`, dead code since the constructor assigns `pricing_lookup or registry_pricing_lookup` — a guard that cannot fail reads as a handled `None` case that was never handled. `tests/test_issue642_type_debt_ratchet.py` fails if the count rises above the ceiling and warns (never fails) if it drops below, which makes the CI Type Check job — which runs `mypy … || echo` and is green whatever mypy says — effectively blocking at the current level while the remaining 71 are burned down. Remaining classes: `arg-type` 26, `assignment` 15, `dict-item` 6, `return-value` 5, `union-attr`/`override`/`index`/`attr-defined` 4 each, `call-overload` 2, `type-var` 1.
...
- **Evidence dispositions no longer vanish under structured findings ([#664](https://github.com/amiable-dev/llm-council/issues/664))** — with `LLM_COUNCIL_STRUCTURED_FINDINGS=true`, *every* ADR-042 evidence item came back `status: "parser_error"` and `council_confirmed: null`, even when the council had evaluated the evidence and its per-item reasoning was sitting in the response. The two features are structurally incompatible as written: structured findings (ADR-051) asks the chairman for one top-level JSON object, so the ADR-042 dispositions fence ends up nested inside that object's `rationale` **string**, where the newlines are the escape sequence `\n` rather than real newlines and the fence regex's mandatory `\n` never matches. `parse_evidence_dispositions` now looks in three places in order — a fence in the raw text (the pre-existing path, unchanged), an `evidence_dispositions` key on the top-level object, then a fence inside any of that object's decoded string values — each step soft-failing, so a genuinely absent block still reports `parser_error` rather than inventing one. Found while reviewing ADR-055 with the council; the hallucinated-id guard and every pre-existing parser behaviour are unchanged and test-pinned.

### Documentation

- **`verify`'s real deadline is published ([#663](https://github.com/amiable-dev/llm-council/issues/663))** — `run_verification` wraps the pipeline in `tier_deadline × VERIFICATION_TIMEOUT_MULTIPLIER (2.0)`, so a `reasoning` verify runs to **1200s**, not the 600s tier budget. The only `MCP_TIMEOUT` guidance in the docs lived in the `consult_council` section and quoted tier budgets, so a user following it set a client timeout at half what a verify can legitimately take — and a client-side kill returns no verdict, no transcript reference and no `unclear_reason`, making it indistinguishable from a hang. The verify guide now carries a per-tier deadline column and its own `MCP_TIMEOUT` numbers, and the consult guidance says it applies to consult. No behaviour change: the 2.0 multiplier was deliberate (raised from 1.5 so stage 3 isn't starved); the defect was never publishing the number to the people who configure around it.
